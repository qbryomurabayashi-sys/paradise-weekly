...
export const MultiUserSelect: React.FC<MultiUserSelectProps> = ({ selectedUsers, onChange, placeholder = "担当者を追加..." }) => {
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      >
        {selectedUsers.map(u => (
          <span key={u.uid} className="flex items-center gap-1 bg-paradise-ocean/10 text-paradise-ocean px-2 py-1 border border-paradise-ocean/20 rounded-md text-[10px] font-bold">
            {u.name}
        ...
        <input 
          ...
          className="flex-1 bg-transparent shrink-0 min-w[60px] outline-none text-xs text-gray-700 py-1 px-1"
        />
      </div>
      ...
      {isOpen && (
        <GlassCard className="absolute top-full left-0 right-0 mt-1 z-50 p-2 max-h-48 overflow-y-auto no-scrollbar shadow-xl border border-white/60">
          {filteredUsers.length === 0 ? (
            <div className="text-center text-xs text-gray-400 py-2 font-bold">該当ユーザーがいません</div>
          ) : (
            filteredUsers.map(u => (
              <div 
                key={u.uid} 
              ...
              >
                ...
                <div className="text-[10px] text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md">{u.storeName} ({u.role})</div>
              </div>
            ))
          )}
        </GlassCard>
